...
function addInitialYields(code = "", yieldKeyword = "yield")
{
	...
	if (match)
	{
		...
		{
			rest = addInitialYields(rest, yieldKeyword)
		...
		{
			args = addInitialYields(args, yieldKeyword)
			rest = addInitialYields(rest, yieldKeyword)
			...
			if (foreword === "return")
			{
				retval = `return (${yieldKeyword} ` + fcall + openbracket + args + closebracket + ")" + rest
			}
			else if (
			...
			else
			{
				...
				retval = `(${yieldKeyword} ` + foreword + fcall + openbracket + args + closebracket + ")" + rest
			}
		}
...
{
	var regex1 = new RegExp(`((?:\\(${yieldKeyword} )+)([\\s\\S]*)`)
	//var regex2 = /((?:\(yield )+)([\s\S]*)/
	...
	while (true)
	{
		...
		code = code.replace(regex1, function(all, yieldcode, rest)
		{
			...
			var yieldscount = yieldcode.length / `(${yieldKeyword} `.length // how many "(yield " in a row
			...
			if (afteryields[0] === ".")
			{
				...
				if (match)
				{
					...
					if (args_match)
					{
						return `(${yieldKeyword} ` + yieldcode + chain + ")" + ".SynchronatorCallbackCatcher(function(){this." + match[1] + "(" + args_match[1] + ",arguments[0])})" + ")" + rest
					}
					else
					{
						return `(${yieldKeyword} ` + yieldcode + chain + ")" + "." + match[1] + "(" + args + ")" + ")" + rest
					}
				}
				else {
...
function removeUnnecesarryYields(code = "", yieldKeyword = "yield")
{
	...
	var pattern = new RegExp(`(function\\s*\\*?\\s*)\\(${yieldKeyword} ([\\s\\S]+)`, "g")
	//var pattern2 = /(function\s*\*?\s*)\(yield ([\s\S]+)/g
...
function makeNormalFunctionsMap(code = "")
{
	...
	let patterns = [
		/(?<!async )(?<!\w)(\s*function(\s+[A-Za-z_$][\w$]*)?\s*\([^\(\){};]*\)\s*){/g, // 1) regular functions pattern. Note that sometimes a function name could be "something_function"
		/(?<!async )(?<!\w)(\([^\(\){};]*\)\s*=>\s*){/g,                                // 2) arrow functions pattern
...
function addSynchronatorCode(code = "", weirdKeyword = "", stringiesMap, currentClassMap)
{
	...
	if (currentClassMap)
	{
		...
		// * fname(args) {
		patterns.push(/(?<frontSymbol>^|[^\w])(?<functionKeyword>)(?<type>\*)\s*(?<functionName>[A-Za-z0-9_$\[\]'"`]+)\s*\((?<functionArguments>[^\(\){};]*)\)(?<emptyString>\s*)(?={)/g)
		patterns.push(/(?<frontSymbol>^|[^\w])(?<type>async)\s+(?<functionName>[A-Za-z0-9_$\[\]'"`]+)\s*\((?<functionArguments>[^\(\){};]*)\)(?<emptyString>\s*)(?={)/g)
	}

	// function * fname(args) | function * (args) {
	patterns.push(/(?<frontSymbol>^|[^\w])(?<functionKeyword>function)\s*(?<type>\*)\s*(?<functionName>[A-Za-z0-9_$]+)?\s*\((?<functionArguments>[^\(\){}]*)\)(?<emptyString>\s*)(?={)/g)
	patterns.push(/(?<frontSymbol>^|[^\w])(?<type>async)\s+(?<functionKeyword>function)\s*(?<functionName>[A-Za-z0-9_$]+)?\s*\((?<functionArguments>[^\(\){}]*)\)(?<emptyString>\s*)(?={)/g)
	...
	for (let i = matches.length - 1; i >= 0; i--)
	{
		...
		let functionKeyword   = match.groups["functionKeyword"] || ""
		...
		let emptyString       = match.groups["emptyString"]       // any space symbols before {
		...
		var wholeFunction = ""
		var matchYields   = (type === "*") ? body.match(/[^\w](yield )/) : body.match(/[^\w](await )/)
