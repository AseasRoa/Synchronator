...
var path_cache = $os.tmpdir() + $path.sep + "galaxia" + $path.sep + "synchronator"
if (!mkdirs(path_cache)) path_cache = null

function addGeneralYields(code)
{
...
{
	//== put yields for chaining functions, like fn().fn() becomes (yield (yield fn()).fn())
...
/**
 * Turns regular JavaScript code into Synchronator-ready code
 * @param code {string} The input JavaScript code
...
var transform = function(code, file)
{
	...
	if (path_cache && file)
	{
		...
		if (exist)
		{
			...
			{
				try {
					return {code: $fs.readFileSync(cache_filename, "utf8")}
				}
				catch (error) {
					console.error(error)
				}
