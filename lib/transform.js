	...
	if (match)
	{
		...
		else
		{
			...
			if (dotNotationDetected == false)
			{
				...
				if (args_match)
				{
					fcall        = "Synchronator.CallbackCatcher(function(){" + ((fcall[0] === ".") ? "this" : "") + fcall + "(" + args_match[1] + ",arguments[0])})"
					args         = ""
					openbracket  = ""
					closebracket = ""
				}
...
function makeNormalFunctionsMap(code = "")
{
	...
	let patterns = [
		...
		/(?<![*0-9])([\s\{\}]\s+?([A-Za-z_$][\w$]*)\s*\([^\(\){};]*\)\s*){/g // 3) methods in class
	]
	...
	for (let p in patterns)
	{
		...
		codetmp += code.substr(offset) // the 1 here os because of the closing }
		code    = codetmp
		codetmp = ""
	}
