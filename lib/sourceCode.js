...
SC.prototype = {
	...
	stringiesEncode: function(code)
	{
		...
		if (positions.length > 0)
		{
			...
			for (var i=0; i<positions.length; i++)
			{
				...
				stringies[word] = code.substring(pos1, pos2+1)
	...
	{
		// I could use this code here:
		/*
		for (var keyword in stringiesMap)
		{
			code = code.replace(keyword, stringiesMap[keyword])
		}
		return code
		*/
		// but it is slow.
		...
		for (var keyword in stringiesMap)
		{
			...
			if (pos > -1)
			{
				codeout += code.substring(offset, pos) + stringiesMap[keyword]
				...
				codeout = codeout.replace(keyword, stringiesMap[keyword])
			}
		}
