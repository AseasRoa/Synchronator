...
function isModuleSynchronatorReady(dirname)
{
	...
	try
	{
		...
		{
			is_module_synchronator_ready = true
		}
	} catch (error)
	{
		is_module_synchronator_ready = true
...
function requireEx()
{
	...
	var retval = new $synchronator((resolve, reject) => {
		...
		{
			// We just want the first path in the while loop to be the current folder, but .dirname() goes one step up
			// That's why arbitrary "/something" folder is appended below - it will be removed in the first iteration
			var tmpPath = $path.resolve(folder + "/something")
		...
		if (!isLocalModule)
		{
			...
			} catch (error)
			{
				...
				{
					// only the last row of the stack is interesting
					let split = error.stack.split("\n")

					console.error(error.message + "\n" + split[split.length - 1])

					// do not resolve or reject here, so the program could hang and not continue further
				}
...
/**
 * Watch file for changes and clear the cache
 * @param {string} path
 * @returns {boolean}
 */
